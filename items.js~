...
function ItemDAO(database) {
	...
	*/
	...
	this.db.collection("item").aggregate(query).
		toArray(function(err, result){
			assert.equal(err, null);
	...
	*/
	...
	this.db.collection("item").aggregate(queryTODB).toArray(function(err, items){
	 	assert.equal(err, null);
		callback(items);
		//console.log(items);		
	 });
	...
	//note that count() function provides callback
	this.db.collection("item").find(queryTODB).count(function(err, count){
	 	assert.equal(err, null);
		callback(count);
		//console.log("count the search query:" + count);		
	});
        //callback(numItems);
    }
	...
	this.db.collection("item").findOneAndUpdate({_id:itemId},{$push:{reviews: reviewDoc}}, function(err, doc){
		assert.equal(err,null);
		//console.log(doc);		
		callback(doc);
	});
